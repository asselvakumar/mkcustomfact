...
def grp(patchgroup, emailaddress)
  ...
  # html Creation
  puppet_report_html = File.open(path_file.to_s, 'w')
  # CSV Creation
  puppet_report_csv = File.open(path_file_csv.to_s, 'w')
  ...
  puppetdb_reports_json.each do |report|
    next unless report['name'] == 'clientcert'
    ...
    next unless grouping == patchgroup.to_s
    # patch group
...
grp patchgroup.to_s, emailaddress.to_s
